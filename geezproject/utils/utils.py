...
async def autocreategroup():
    LOGS.info("__AUTO CREATE GROUP__\n __⚠️ Warning ⚠️ Jangan Dihapus__\n\n__Mohon Menunggu Sebentar. Sedang Membuat Group Untuk geezproject Anda!!__")
...
async def autobot():
    ...
    if isdone.startswith("Sorry,"):
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await bot.send_file(bf, "geezproject/resources/logo.jpg")
            await asyncio.sleep(3)
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"geezproject/modules/{shortname}.py")
        name = "geezproject.modules.{}".format(shortname)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["geezproject.modules." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"geezproject/modules/assistant/{shortname}.py")
        name = "geezproject.modules.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["geezproject.modules.assistant" + shortname] = mod
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"geezproject.modules.{shortname}"
