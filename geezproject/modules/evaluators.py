...
#
"""geezproject module for executing code and terminal commands from Telegram."""
...
async def _(event):
    ...
    if expression in ("geezproject.session", "config.env"):
    ...
    if len(final_output) > 4096:
        ...
        with io.BytesIO(str.encode(geez)) as out_file:
...
async def run(event):
    ...
    if code in ("geezproject.session", "config.env"):
        return await event.edit("`Itu operasi yang berbahaya! Tidak diperbolehkan!`")
...
async def terminal_runner(event):
    ...
    if command in ("geezproject.session", "config.env"):
        return await event.edit("`Itu operasi yang berbahaya! Tidak diperbolehkan!`")
...
CMD_HELP.update(
    {
        "json": f"**Plugin : **`json`\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}json` <reply ke pesan>\
