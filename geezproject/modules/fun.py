...
from geezproject.utils import bash, progress
...
CMD_HELP.update(
    {
        "rgif": f"**Plugin : **`rgif`\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}gif` <sambil reply ke media>\
        \n  ❍▸ : **Untuk mengubah gambar jadi gif memutar.\
    "
    }
)
...
CMD_HELP.update(
    {
        "fun": f"**Plugin : **`fun`\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}rst` <text>\
        \n  ❍▸ : **Untuk membuat stiker teks dengan templat stiker acak.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}honka` <text>\
        \n  ❍▸ : **Untuk membuat stiker teks dengan templat stiker Honka bot.\
    "
    }
)
