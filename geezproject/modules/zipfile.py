...
#
# Port from UniBorg to geezproject by yincen17
...
from geezproject import CMD_HELP, TEMP_DOWNLOAD_DIRECTORY, ZIP_DOWNLOAD_DIRECTORY, bot
from geezproject.events import geez_cmd
from geezproject.utils import progress
...
@bot.on(geez_cmd(outgoing=True, pattern=r"compress(?: |$)(.*)"))
async def _(event):
...
@bot.on(geez_cmd(outgoing=True, pattern=r"addzip(?: |$)(.*)"))
...
@bot.on(geez_cmd(outgoing=True, pattern=r"upzip(?: |$)(.*)"))
...
@bot.on(geez_cmd(outgoing=True, pattern=r"rmzip(?: |$)(.*)"))
...
CMD_HELP.update(
    {
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}compress` **[optional: <reply to file>]**\
        \n  ↳ : **make files to zip.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}addzip` **<reply to file>**\
        \n  ↳ : **add files to zip list.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}upzip` **[optional: <zip title>]**\
        \n  ↳ : **upload zip list.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}rmzip` **[optional: <zip title>]**\
        \n  ↳ : **clear zip list.\
    "
    }
)
