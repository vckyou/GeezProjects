...
#
""" geezproject module containing commands for keeping notes. """
...
async def notes_active(svd):
    ...
    try:
        from geezproject.modules.sql_helper.notes_sql import get_notes
...
async def remove_notes(clr):
    ...
    try:
        from geezproject.modules.sql_helper.notes_sql import rm_note
    except AttributeError:
...
async def add_note(fltr):
    ...
    try:
        from geezproject.modules.sql_helper.notes_sql import add_note
    except AttributeError:
...
async def incom_note(getnt):
    ...
    try:
        if not (await getnt.get_sender()).bot:
            try:
                from geezproject.modules.sql_helper.notes_sql import get_note
            except AttributeError:
                return
...
CMD_HELP.update(
    {
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `#<notename>`\
        \n  ❍▸ : **Mendapat catatan yang ditentukan.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}save` <notename> <notedata> atau balas pesan dengan .save <notename>\
        \n  ❍▸ : **Menyimpan pesan yang dibalas sebagai catatan dengan notename. (Bekerja dengan foto, dokumen, dan stiker juga!).\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}notes`\
        \n  ❍▸ : **Mendapat semua catatan yang disimpan dalam obrolan \
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}clear` <notename>\
        \n  ❍▸ : **Menghapus catatan yang ditentukan. \
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}rmbotnotes` <marie / rose>\
        \n  ❍▸ : **Menghapus semua catatan bot admin (Saat ini didukung: Marie, Rose, dan klonnya.) Dalam obrolan.\
    "
    }
)
