...
from geezproject import BLACKLIST_CHAT, BOTLOG_CHATID, CLEAN_WELCOME
...
async def welcome_to_chat(event):
    try:
        from geezproject.modules.sql_helper.welcome_sql import (
...
async def save_welcome(event):
    ...
    try:
        from geezproject.modules.sql_helper.welcome_sql import add_welcome_setting
    except AttributeError:
...
async def show_welcome(event):
    try:
        from geezproject.modules.sql_helper.welcome_sql import get_current_welcome_settings
    except AttributeError:
...
async def del_welcome(event):
    try:
        from geezproject.modules.sql_helper.welcome_sql import rm_welcome_setting
    except AttributeError:
        return await event.edit("`Running on Non-SQL mode!`")
...
CMD_HELP.update(
    {
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}setwelcome` <pesan welcome> atau balas ke pesan ketik `.setwelcome`\
        \n  •  **Function : **Menyimpan pesan welcome digrup.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}checkwelcome`\
        \n  •  **Function : **Check pesan welcome yang anda simpan.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}rmwelcome`\
