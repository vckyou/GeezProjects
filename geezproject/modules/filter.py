...
from geezproject import BLACKLIST_CHAT, BOTLOG_CHATID
from geezproject import CMD_HANDLER as cmd
from geezproject import CMD_HELP, bot
from geezproject.events import geez_cmd
from geezproject.modules.sql_helper.filter_sql import (
    add_filter,
    get_filters,
    remove_all_filters,
    remove_filter,
...
@bot.on(geez_cmd(outgoing=True, pattern="filter (.*)"))
...
@bot.on(geez_cmd(outgoing=True, pattern="filters$"))
...
@bot.on(geez_cmd(outgoing=True, pattern="stop ([\s\S]*)"))
...
@bot.on(geez_cmd(outgoing=True, pattern="rmallfilters$"))
...
CMD_HELP.update(
    {
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}filters`\
        \n  ❍▸ : **Melihat filter geezproject yang aktif di obrolan.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}filter` <keyword> <balasan> atau balas ke pesan ketik `.filter` <keyword>\
        \n  ❍▸ : **Membuat filter di obrolan, Bot Akan Membalas Jika Ada Yang Menyebut 'keyword' yang dibuat. Bisa dipakai ke media/sticker/vn/file.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}stop` <keyword>\
        \n  ❍▸ : **Untuk Nonaktifkan Filter.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}delfilterbot` <marie/rose>\
        \n  ❍▸ : **Menghapus semua filter yang ada di bot grup (Saat ini bot yang didukung: Marie, Rose.) dalam obrolan.\
    "
    }
)
