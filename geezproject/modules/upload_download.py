...
""" geezproject module which contains everything related to
...
from geezproject import CMD_HELP, LOGS, TEMP_DOWNLOAD_DIRECTORY, bot
from geezproject.events import geez_cmd
from geezproject.utils import humanbytes, progress, run_cmd
from geezproject.utils.FastTelethon import download_file, upload_file


@bot.on(geez_cmd(pattern=r"download(?: |$)(.*)", outgoing=True))
async def download(target_file):
    """For .download command, download files to the geezproject's server."""
...
CMD_HELP.update(
    {
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}download` <link|filename> atau reply ke media\
        \n  ↳ : **Untuk mengdownload file ke server.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}upload`\
        \n  ↳ : **Mengunggah file yang disimpan secara lokal ke obrolan.\
    "
    }
)
