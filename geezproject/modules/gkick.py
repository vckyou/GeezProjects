...
async def gspide(rk):
    ...
    await rkp.edit(f"`{owner} Memproses Global Kick!`")
    ...
    if user:
        if user.id == DEVS:
            return await rkp.edit(f"`Terjadi Kesalahan Karna {owner} Pembuat Saya🙏`")
        try:
        ...
        for i in testrk:
            try:
                ...
                a += 1
                await rkp.edit(
                    f"`{owner} Memproses Global Kick Gkicked di {a} Group Chats..`"
                )

            except BaseException:
                b += 1
    else:
        await rkp.edit(f"`{owner} Balas Kepesan`")
...
CMD_HELP.update(
    {
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}gkick` <alasan>\
        \n  ❍▸ : **kick pengguna secara global dari semua Administrasi Grup di mana Anda berada.\
    "
    }
)
