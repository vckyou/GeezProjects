...
async def fban(event):
    ...
    try:
        from geezproject.modules.sql_helper.fban_sql import get_flist
...
async def addf(event):
    ...
    try:
        from geezproject.modules.sql_helper.fban_sql import add_flist
    except IntegrityError:
        return await event.edit("**Running on Non-SQL mode!**")
...
async def delf(event):
    ...
    try:
        from geezproject.modules.sql_helper.fban_sql import del_flist
    except IntegrityError:
        return await event.edit("**Running on Non-SQL mode!**")
...
async def clearf(event):
    ...
    try:
        from geezproject.modules.sql_helper.fban_sql import del_flist_all
    except IntegrityError:
        return await event.edit("**Running on Non-SQL mode!**")
...
CMD_HELP.update(
    {
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `.fban` <id/username/reply> <reason>\
        \n  ❍▸ : **Membanned user dari federasi yang terhubung.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `.dfban` <id/username/reply> <reason>\
        \n  ❍▸ : **Membanned user dari federasi yang terhubung dengan menghapus pesan yang dibalas.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `.unfban` <id/username/reply> <reason>\
        \n  ❍▸ : **Membatalkan Federations Banned\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `.addf` <nama>\
        \n  ❍▸ : **Menambahkan grup saat ini dan menyimpannya sebagai <nama> di federasi yang terhubung. Menambahkan satu grup sudah cukup untuk satu federasi.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `.delf`\
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `.clearf`\
        \n  ❍▸ : **Menghapus dari semua federasi yang terhubung. Gunakan dengan hati-hati.\
    "
    }
)
