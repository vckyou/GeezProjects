...
# Ported to geezproject by @Mayur_Karaniya
...
async def download(event):
    ...
    if GIT_REPO_NAME is None:
        await event.edit("`Please ADD Proper Github Repo Name of your geezproject`")
        return
...
async def git_commit(file_name, mone):
    ...
    file_name = "geezproject/modules/" + file_name
    if create_file:
        file_name = file_name.replace("./geezproject/temp/", "")
        print(file_name)
        try:
            repo.create_file(
            ...
            ccess = ccess.strip()
            await mone.edit(
                f"`Commited On Your Github Repo`\n\n[Your Modules](https://github.com/{ccess}/tree/sql-extended/geezproject/modules/)"
            )
...
CMD_HELP.update(
    {
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `.gcommit`\
        \n  ❍▸ : **Plugin Pengunggah File GITHUB untuk geezproject. Otomatisasi Heroku harus Diaktifkan. Untuk orang pemalas\
