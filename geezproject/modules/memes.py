...
""" geezproject module for having some fun with people. """
...
from geezproject.utils import edit_delete, get_user_from_event
...
async def _(cowmsg):
    """For .cowsay module, geezproject wrapper for cow which says things."""
...
async def _(react):
    """Make your geezproject react to everything."""
...
CMD_HELP.update(
    {
        ...
        "\nUsage: UwU"
        f"\n\n>`{cmd}react`"
        "\nUsage: Buat geezproject Anda bereaksi terhadap semuanya."
        f"\n\n>`{cmd}slap`"
...
CMD_HELP.update(
    {
        "scam": f"**Plugin : **`scam`\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}scam` <action> <detik>\
        \n  ❍▸ : **Untuk menunjukkan tindakan palsu dengan jangka waktu (sama seperti fakeaction)\
