...
from geezproject.modules.sql_helper import snips_sql as sql
from geezproject.utils import edit_delete, edit_or_reply, geez_cmd, reply_id


@geez_cmd(pattern=r"\#(\S+)")
...
@geez_cmd(pattern="delcustom(?:\\s|$)([\\s\\S]*)")
async def _(event):
...
CMD_HELP.update(
    {
        ...
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}custom` <nama> <data> atau membalas pesan dengan .custom <nama>\
        \n  ❍▸ : **Menyimpan pesan custom (catatan global) dengan nama. (bisa dengan gambar, docs, dan stickers!)\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}customs`\
        \n  ❍▸ : **Mendapat semua customs yang disimpan.\
        \n\n  𝘾𝙤𝙢𝙢𝙖𝙣𝙙 :** `{cmd}delcustom` <nama_custom>\
        \n  ❍▸ : **Menghapus custom yang ditentukan.\
    "
    }
)
